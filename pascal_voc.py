...
class pascal_voc(object):
    ...
    def get(self):
        ...
        while count < self.batch_size:
            ...
            gt_box_len=len(self.gt_labels[self.cursor]['boxes'])
            gt_box = np.empty((gt_box_len, 4), dtype=np.float32)
            gt_box[:, [0, 2]] = self.gt_labels[self.cursor]['boxes'][:, [0, 2]] * image_scale[0]
            gt_box[:, [1, 3]] = self.gt_labels[self.cursor]['boxes'][:, [1, 3]] * image_scale[1]
    ...
    def prepare(self, imdb_name):
        ...
        if self.flipped:
            print(
                'Appending horizontally-flipped training examples ...')  # {'boxes':boxes, 'gt_classs':gt_classes, 'imname':imname}
    ...
    def load_pascal_annotation(self, index, imdb_name):
        ...
        return {'boxes': boxes, 'gt_classs': gt_classes, 'imname': imname, 'flipped': False, 'image_size': size_info,
                'image_index': index, 'diff': difficult}
    ...
    def prep_im_for_blob(self, im, pixel_means, target_size, max_size):
        ...
        im -= pixel_means
        im_shape = im.shape
        im_scale_y = target_size/ im_shape[0]
        im_scale_x = target_size/ im_shape[1]
        # Prevent the biggest axis from being more than MAX_SIZE
...
if __name__ == '__main__':
    pascal = pascal_voc(cfg.train_imdb_name, 'train', fliped=True)
    tf_blob = pascal.get()
